	...
	if (inode && is_bad_inode(inode))
		...
		if (!ret) {
			...
			if (outarg.nodeid != get_node_id(inode)) {
				...
				pr_info("%s %d outarg.nodeid %d %d %d\n",__func__, __LINE__, outarg.nodeid,get_node_id(inode),flags);
				goto invalid;
			}
			spin_lock(&fc->lock);
			fi->nlookup++;
		...
		if (ret || fuse_invalid_attr(&outarg.attr) ||
		    ...
		    pr_info("%s %d inode %d ret %d\n",__func__, __LINE__, flags,ret);
			pr_info("%s %d  %d ret %d %d\n",__func__, __LINE__, fuse_invalid_attr(&outarg.attr),outarg.attr.mode,inode->i_mode);
			pr_info("%s %d  %d ret %u %u\n",__func__, __LINE__, fuse_invalid_attr(&outarg.attr),outarg.attr_valid,outarg.entry_valid);
			goto invalid;
		}
